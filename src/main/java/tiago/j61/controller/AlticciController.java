...
public class AlticciController {
	...
	@GetMapping(path = "alticciValue/{index}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<?> getValue(@PathVariable int index) {
		try {

			AlticciResponseDto response = alticciFacada.getValueOfIndex(index);
